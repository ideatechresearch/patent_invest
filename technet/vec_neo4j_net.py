...
def cosine_sim_top(co_ids_vec, top_n=50):
...
class VRelationships:
    ...
    def load(self, graph, data):
        self.graph = graph
        self.data = data
        print('数据导入:', self.data.columns, self.data.index, self.data.shape)
        ...
        while depth <= max_depth and len(words_depth) <= max_node:
            try:
                ...
                if not (self.graph and create):
                    ...
                    node_name = f"Co_{co}"
                    co_vec = self.data.loc[co]  # arr
                    for x in similar_words_next:
                        word_node = CreateNode(x[0], words_depth[x[0]], self.graph, node_name,
                                               similar=float(cosine_sim(co_vec, self.data.loc[x[0]])))  # w=x[0]
                        relationships = [Relationship(word_node, "SIMILAR_TO",
                                                      CreateNode(y[0], words_depth[y[0]], self.graph, node_name,
                                                                 similar=float(
                                                                     cosine_sim(co_vec, self.data.loc[y[0]]))),
...
if __name__ == '__main__':
    ...
    co_ids_vec = pd.read_parquet('data/patent_co_vec_23.parquet')
    vr = VRelationships()
    vr.load(graph=graph, data=co_ids_vec)

    while True:
        res = input('输入:')
        txt = res.strip(string.punctuation).strip().lower()
        if txt == 'exit' or txt == 'none':
            break
        words_depth, relationships_edges = vr.create_relationships(txt, width=3,
                                                                    max_depth=3,
                                                                    duplicate=3,
                                                                    create=1,
                                                                    max_node=800)
        # CreateCoSimilarityRelationship('通联数据股份公司', co_ids_vec, width=3, max_depth=2, wid='通联数据', duplicate=0,
        #                                graph=graph)
        print(f'Co_{txt}:{words_depth}')
