 ...
 ''',
  ...
  ''',
    ...
    **示例输出格式（仅供参考）**：
    ...
    'search_prompt': '''你是一个信息重排助手，任务是根据用户的查询意图（querys）对搜索结果（search_hit）进行语义相关性重排，并返回多个最相关的文本内容。
...
''',
    'legal_person':'''
    你是企业变更信息抽取助手，任务是从工商变更记录中提取所有涉及“法定代表人变更”及其相关的自然人或公司信息。

    请仅提取因法定代表人变更所引发的关键角色变更，包括但不限于：法定代表人、经理、财务负责人、联络员、投资人（如涉及股东或出资人变更）。  
    忽略与法定代表人无直接关联的其他高管调整（如监事、普通董事等）。
    
    输出格式为 JSON 数组，每个元素为一个对象，包含以下字段：
    - name：人员或公司名称
    - role：其在企业中的职务，如“法定代表人”、“经理”、“财务负责人”等
    - type：类型（自然人或公司）
    
    输出示例：
    ```json
    [
      {"name": "李天霸", "role": "法定代表人", "type": "自然人"},
      {"name": "向孟奇", "role": "法定代表人", "type": "自然人"},
      
      {"name": "李天霸", "role": "经理", "type": "自然人"},
      {"name": "向孟奇", "role": "经理", "type": "自然人"},
      
      {"name": "李天霸", "role": "联络员", "type": "自然人"},
     {"name": "向孟奇", "role": "联络员", "type": "自然人"},
      
      {"name": "云南茗蕴商业管理有限公司", "role": "投资人", "type": "公司"},
      {"name": "向孟奇", "role": "投资人", "type": "自然人"}
      ...
    ]
    ```
    仅保留发生变更的主体及其职务信息，无需提取无关人员。注意：如果存在同一角色由不同人员交替担任，需分别记录。
    '''
}
