QUERY_TASKS = [
    ...
    },
    {
        ...
        """,
        "extra_question": """
        请根据以下信息判断该法人名下企业是否存在空壳公司或异常行为，并结合企业的数量、年报披露情况、经营状态、对外投资关系等维度进行分析。

        请返回一个评分（0~100分）及对应的分析结果，评分含义如下：
        - 0~40分：法人名下企业整体正常，无明显异常；
        ...
        ```
        如有多个企业存在共性问题（如无年报、无经营活动但保留法人身份），请明确指出并说明是否构成关联性或操控痕迹。
        """,
        'field_mapping': {
